...
<font size=5>**问题**：</font>在总计算延迟受限、个体用户下载数据总能量受限，以及本地计算处理的频率受限的条件下，移动用户能量最小化是一个凸问题，可以用经典的拉格朗日对偶法找到最优解。
...
$$ L(\beta , \omega , \sigma , \nu,t^{ul}_{u,S},t^{ul}_{u},t^{dl}_{u},t^{dl},D^L_u,D^I_{u,S})= \sum_{u \in U}\frac{t^{ul}_{u,S}}{|h_u|^2}f(\frac{D^I_{u,S}}{t^{ul}_{u,S}})+\sum_{u \in U}\kappa_0\frac{(\lambda_0D^L_u)^3}{(t^C_{u,L})^2}+\sum_{u \in U}\frac{t^{ul}_{u}}{|h_u|^2}f(\frac{D^I_{u}-D^I_{S}-D^L_{u}}{t^{ul}_{u}})+\sum_{u \in U}(t^{dl}_{u,S}+t^{dl}_{u})\rho^{dl}_u+$$ 
$$ \sum_{u=1}^{U} \beta_u(t^{ul}_{u,S}+t^{ul}_{u}-T_{max}+t^{dl}_S+t^{dl})+ \sum_{u=1}^U\omega_{u}(\lambda_0D^L_u-t^C_{u,Lf_{u,max}})+\sum_{u=1}^{U}\sigma_u(t^{dl}_u-t^{dl})+ \nu [ \sum_{u=1}^{U} \frac{t^{dl}_u}{|g_u|^2} f(\frac{a_0(D^I_{u}-D^I_{S}-D^L_{u})}{t^{dl}_{u}})-E_{max}]  $$
...
$$ -\beta_u |h_u|^2= N^0(2^{\frac{D^I_{u,S}}{W^{ul}_ut^{ul}_{u,S}}}-1)-t^{ul}_{u,S}(ln2 N^02^{\frac{D^I_{u,S}}{W^{ul}_ut^{ul}_{u,S}}}\frac{D^I_{u,S}}{W^{ul}_u(t^{ul}_{u,S})^2})$$
$$ \beta_u |h_u|^2= -N^0(2^{\frac{D^I_{u,S}}{W^{ul}_ut^{ul}_{u,S}}}-1)+t^{ul}_{u,S}(ln2 N^02^{\frac{D^I_{u,S}}{W^{ul}_ut^{ul}_{u,S}}}\frac{D^I_{u,S}}{W^{ul}_u(t^{ul}_{u,S})^2})$$
$$ \frac{\beta_u |h_u|^2}{N^0}= -(2^{\frac{D^I_{u,S}}{W^{ul}_ut^{ul}_{u,S}}}-1)+ln2t^{ul}_{u,S} 2^{\frac{D^I_{u,S}}{W^{ul}_ut^{ul}_{u,S}}}\frac{D^I_{u,S}}{W^{ul}_u(t^{ul}_{u,S})^2}$$
$$ \frac{\beta_u |h_u|^2}{N^0}-1= -2^{\frac{D^I_{u,S}}{W^{ul}_ut^{ul}_{u,S}}}+ln2t^{ul}_{u,S} 2^{\frac{D^I_{u,S}}{W^{ul}_ut^{ul}_{u,S}}}\frac{D^I_{u,S}}{W^{ul}_u(t^{ul}_{u,S})^2}$$
$$ \frac{\beta_u |h_u|^2}{N^0}-1= -2^{\frac{D^I_{u,S}}{W^{ul}_ut^{ul}_{u,S}}}+ 2^{\frac{D^I_{u,S}}{W^{ul}_ut^{ul}_{u,S}}}\frac{ln2D^I_{u,S}}{W^{ul}_ut^{ul}_{u,S}}$$
$$ \frac{\beta_u |h_u|^2}{N^0}-1=  2^{\frac{D^I_{u,S}}{W^{ul}_ut^{ul}_{u,S}}}\bigg(\frac{ln2D^I_{u,S}}{W^{ul}_ut^{ul}_{u,S}}-1\bigg)$$
$$ 2^{-\frac{1}{ln2}}(\frac{\beta_u |h_u|^2}{N^0}-1)=  2^{\frac{D^I_{u,S}}{W^{ul}_ut^{ul}_{u,S}}-\frac{1}{ln2}}\bigg(\frac{ln2D^I_{u,S}}{W^{ul}_ut^{ul}_{u,S}}-1\bigg)$$
其中
$$ -\frac{1}{ln2}=-\frac{log_2e}{log_22}=-log_2e$$
所以：
$$ 2^{-log_2e}(\frac{\beta_u |h_u|^2}{N^0}-1)=  2^{\frac{D^I_{u,S}}{W^{ul}_ut^{ul}_{u,S}}-\frac{1}{ln2}}\bigg(\frac{ln2D^I_{u,S}}{W^{ul}_ut^{ul}_{u,S}}-1\bigg)$$
$$ \frac{1}{e}(\frac{\beta_u |h_u|^2}{N^0}-1)=  2^{\frac{D^I_{u,S}}{W^{ul}_ut^{ul}_{u,S}}-\frac{1}{ln2}}\bigg(\frac{ln2D^I_{u,S}}{W^{ul}_ut^{ul}_{u,S}}-1\bigg)$$
借用博朗W函数：
$$W_0(x)e^{W_0(x)}=x$$
稍稍变形：
$$e^{W_0(x)}=\frac{x}{W_0(x)}$$
$${W_0(x)}=ln(\frac{x}{W_0(x)})$$
$${W_0(x)}=\frac{log_2(\frac{x}{W_0(x)})}{log_2e}$$
$${W_0(x)}log_2e=log_2(\frac{x}{W_0(x)})$$
$$2^{{W_0(x)}log_2e}=\frac{x}{W_0(x)}$$
$$W_0(x)2^{{W_0(x)}log_2e}=x$$

原始：
$$ \frac{1}{e}(\frac{\beta_u |h_u|^2}{N^0}-1)=  2^{(\frac{ln2D^I_{u,S}}{W^{ul}_ut^{ul}_{u,S}}-1)\frac{1}{ln2}}\bigg(\frac{ln2D^I_{u,S}}{W^{ul}_ut^{ul}_{u,S}}-1\bigg)$$
$$ \frac{1}{e}(\frac{\beta_u |h_u|^2}{N^0}-1)=  2^{(\frac{ln2D^I_{u,S}}{W^{ul}_ut^{ul}_{u,S}}-1)log_2e}\bigg(\frac{ln2D^I_{u,S}}{W^{ul}_ut^{ul}_{u,S}}-1\bigg)$$

显然：
$$ x = \frac{1}{e}(\frac{\beta_u |h_u|^2}{N^0}-1)$$
$$ W_0(x) = \frac{ln2D^I_{u,S}}{W^{ul}_ut^{ul}_{u,S}}-1$$

可以求出我们的原始变量：
$$ t^{ul}_{u,S} = \frac{ln2D^I_{u,S}}{W^{ul}_u(W_0(x)+1)}$$
$$ t^{ul}_{u,S} = \frac{ln2D^I_{u,S}}{W^{ul}_u(W_0(\frac{1}{e}(\frac{\beta_u |h_u|^2}{N^0}-1))+1)}$$
我们将这个解记作：$\hat{t}^{ul}_{u,S}$
同理可以计算其他的原始变量：
$$ \hat{t}^{ul}_{u} = \frac{ln2(D^I_{u}-D^I_{S}-D^L_{u})}{W^{ul}_u(W_0(\frac{1}{e}(\frac{\beta_u |h_u|^2}{N^0}-1))+1)}$$
$$ \hat{t}^{dl}_{u} = \frac{ln2(a_0)^2(D^I_{u}-D^I_{S}-D^L_{u})}{W^{ul}_u(W_0(\frac{1}{e}(\frac{ (\rho^{dl}_u+\sigma_u)  |g_u|^2}{\nu N^0}-1))+1)}$$
$$ \hat{D}^L_u=min\bigg\{T_{max}\sqrt{[\frac{N_0ln2}{3 \kappa_0\lambda_0^3}(\frac{2^{\frac{\hat{r}^{ul}_u}{W^{ul}_u}}}{W^{ul}_{u}|h_u|^2})]^+\frac{\nu a_02^{\frac{a_0 \hat{r}^{dl}_u}{W^{dl}_u}}}{W^{dl}_{u}|g_u|^2}},D^I_u-D^I_S\bigg\}$$
其中：
...
$$ \hat{t}^{dl}=\begin{cases}
   ...
   T_{max}-t^{dl}_S &\text{if } otherwise
\end{cases}$$
接下来我们将求每个用户最佳的共享数据卸载比特数，我们借助下面的引理：
对于用户u来说最佳的卸载比特数可以表示为：
$$ \hat{D}^{I}_{u,S}=\begin{cases}
   D^I_S &\text{if } \hat{u}=arg \min\limits_{1 \le u \le U} \Delta_u\\
   0 &\text{if } otherwise
\end{cases}$$
其中：
